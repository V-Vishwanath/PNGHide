...
def get_data():
    ...
    password = tk.Entry(frame, textvar=pass_var, show='*',
                        width=20, font=('Times new Roman', 13), justify='center')
    ...
    label = tk.Label(frame, text=message, font=LABEL_FONT,
                     fg='green', bg='powderblue')
    label.place(relx=0.04, rely=0.43)
    tk.Label(frame, text='Question : ', font=LABEL_FONT, fg='blue',
             bg='powderblue').place(relx=0.33, rely=0.50)
    question = tk.Entry(frame, textvar=question_var, width=20, font=(
        'Times new Roman', 13), justify='center')
    question.place(relx=0.46, rely=0.51)
    tk.Label(frame, text=' Answer  : ', font=LABEL_FONT, fg='blue',
             bg='powderblue').place(relx=0.33, rely=0.56)
    answer = tk.Entry(frame, textvar=answer_var, show='*',
                      width=20, font=('Times new Roman', 13), justify='center')
    ...
    password = tk.Entry(frame, textvar=key_var, width=20,
                        font=('Times new Roman', 13), justify='center')
    password.place(relx=0.37, rely=0.75)

    button = tk.Button(frame, text='Configure', command=save_data,
                       font=BUTTON_FONT, fg='white', bg='red')
...
def save_data():
    ...
    if temp_data['password'] == '':
        messagebox.showerror(
            title='Password empty', message='Passwords cannot be empty. Please try again!')
        return
    ...
    if l is 0:
        messagebox.showerror(
            title='Key error', message='Key cannot be empty. Please try again!')
        return
    if l < 3 or l > 6:
...
def open_screen(event):
    ...
    label = tk.Label(frame, text='Enter password', font=('Comic sans MS', 25, 'bold'), fg='green',
                     bg='powderblue')
    label.place(relx=0.33, rely=0.38)
    password = tk.Entry(frame, textvar=pass_var, show='*',
                        width=20, font=('Times new Roman', 15), justify='center')
    password.place(relx=0.36, rely=0.50)

    password.focus_set()
    password.bind('<Return>', check_password)

    label = tk.Label(frame, text='Forgot password ?',
                     cursor='hand2', font='Arial 11 underline', bg='powderblue')
...
def forgot_password(event):
    ...
    label = tk.Label(frame, text='Question : ' +
                     data['question'], font=LABEL_FONT, bg='powderblue', justify='center')
    label.place(relx=0.35, rely=0.4)
    tk.Label(frame, text=' Answer  : ', font=LABEL_FONT,
             bg='powderblue', justify='center').place(relx=0.35, rely=0.45)
    answer = tk.Entry(frame, textvar=answer_var, show='*',
                      width=20, font=('Times new Roman', 13), justify='center')
    answer.place(relx=0.47, rely=0.46)
    answer.bind('<Return>', check_answer)

    label = tk.Label(frame, text='Back to login screen',
                     cursor='hand2', font='Arial 11 underline', bg='powderblue')
    label.place(relx=0.40, rely=0.58)
...
def check_answer(event):
    ...
    if answer == '':
        messagebox.showerror(
            title='Empty Field', message='Answer cannot be empty. Please try again!')
        return
...
def reset_password():
    ...
    label = tk.Label(frame, text='Enter new password', font=('Comic sans MS', 25, 'bold'), fg='green',
                     bg='powderblue')
...
def save_password(event):
    ...
    messagebox.showinfo(title='Operation Successful',
...
def check_password(event):
    ...
    if pass_var.get() != data['password']:
        messagebox.showerror(
            title='Login Failed', message='That password is incorrect. Please try again.')
        return
...
def init_gui():
    ...
    image_frame = tk.Frame(frame, width=400, height=400,
                           bg='grey', borderwidth=6, relief=tk.RIDGE)
    img_button = tk.Button(
        image_frame, text='Click here to choose image', command=load_img)
    ...
    tk.Label(encode_frame, text='Enter message here:',
             font=LABEL_FONT, fg='blue').place(relx=0.23, rely=0.05)
    input_frame = tk.Frame(encode_frame, width=325, height=210)
    encode = tk.Text(input_frame)
    button = tk.Button(encode_frame, text='Encrypt', command=encrypt,
                       width=10, font=BUTTON_FONT, fg='white', bg='red')
    input_frame.place(relx=0.04, rely=0.15)
    encode.place(relx=0, rely=0)
    button.place(relx=0.34, rely=0.7)

    decode_frame = tk.Frame(nb)
    button = tk.Button(decode_frame, text='Decrypt', command=decrypt,
                       width=10, font=BUTTON_FONT, fg='white', bg='red')
...
def load_img():
    global IMAGE, EXTENSION, IMAGE_LOADED

    img = filedialog.askopenfile(
        initialdir='/', title='Choose image file', filetypes=FILETYPES)
...
def encrypt():
    if IMAGE_LOADED is False:
        messagebox.showerror(title='Image not selected',
                             message='Please choose an image to encrypt!')
        return
    ...
    file = filedialog.asksaveasfilename(initialdir='/', title='Choose save location',
...
if __name__ == '__main__':
    if not path.exists('config.pickle'):
        data = {'initialized_user': False, 'password': '',
                'question': '', 'answer': '', 'key': ''}
